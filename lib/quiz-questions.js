...
var QuizQuestions = module.exports = React.createClass({
	...
	render: function() {
		var Questions = this.props.questions.map(function (question, index){
			var updateThisQuestion = function () {
			...
			return (
				<TabView
					key={ index }
					initialSelection={0}
					tabLabels={ [{ labels.questionTabLabel }, { labels.answerTabLabel }] }
					tabPanels={ [<QuestionPanel removeQuestion={ this.props.removeQuestion } question={ question } updateQuestion={ updateThisQuestion } />, <AnswerPanel resultTitles={ this.props.resultTitles } updateAnswer={ updateThisAnswer } answers={ question.answers } title={ question.prompt } />]}
		...
		if ( this.props.questions.length < 9) {
			var addNewQuestion = <button type="button" id="addNewQuestion" className="editorSecondaryBtn" onClick={this.props.addQuestion}>{ labels.addNewQuestion }</button>
		}
